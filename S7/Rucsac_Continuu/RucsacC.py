...
import copy
from FunctiiCrossoverIndivizi import crossover_singular, crossover_simplu
...
def gen(fc,fv,max,dim):
    ...
    for i in range(dim):
        gata=False
        while gata == False:
            #genereaza candidatul x cu elemente în [0,1]
...
def crossover_populatie(pop,cal,c,v,max,pc,alpha):
    dim=len(pop)
    n=c.size
    ...
    for i in range(0,dim-1,2):
        ...
        if r<=pc:
            # crossover x cu y - singular sau simplu: mai potrivit aici
...
def mutatie_populatie(pop,cal, c,v,max,pm,sigma):
...
##ALGORITMUL GENETIC PENTRU REZOLVAREA PROBLEMEI CONTINUE A RUCSACULUI
...
def GA(fc,fv,cmax,dim,NMAX,pc,pm,alpha, sigma):
    ...
    while it<NMAX and not gata:
        ...
        # RECOMBINAREA
        pop_o, cal_o = crossover_populatie(spop,sval,c, v, cmax, pc, alpha)
        # MUTATIA
        pop_mo,cal_mo = mutatie_populatie(pop_o,cal_o,c,v,cmax,pm, sigma)
...
if __name__=="__main__":
    sol,val=GA("cost1.txt","valoare1.txt",50,400,1000,0.8,0.1,0.3, 0.1)