...
def intensitate_legaturi(matrice=None, dec=2, titlu='Intensitate legaturi',
            color='Oranges'):
    plt.figure(num=titlu, figsize=(12, 8))
    plt.title(label=titlu, fontsize=12,
              verticalalignment='bottom', color='Blue')
    # asumam ca primit ca parametru
    # un numpy.ndarray sau pandas.DataFrame
    sb.heatmap(data=np.round(a=matrice, decimals=dec),
               cmap=color, annot=True)


def cercul_corelatiilor(R2=None, V1=0, V2=1, dec=2,
                        titlu='Cercul corelatiilor'):
    # putem primi ca tip de parametru in R2 fie un numpy.ndarray,
    # fie un pandas.DataFrame
    plt.figure(num=titlu + ' intre ' + 'Componenta ' + str(V1+1) +
              ' si ' + 'Componenta ' + str(V2+1), figsize=(8, 7))
    plt.title(label=titlu + ' intre ' + 'Componenta ' + str(V1+1) +
              ' si ' + 'Componenta ' + str(V2+1),
              fontsize=12,
